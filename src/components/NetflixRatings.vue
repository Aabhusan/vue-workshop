    ...
    <div class="card">
      <div class="filters">
        <div>
          ...
          <div class="actions">
            ...
            <button @click="decending" class="btn">Highest rated</button>
          </div>
        </div>
        <div class="search">
      ...
      <div class="content">
        <table class="table">
          ...
          <tbody>
            <tr v-for="(movie, index) in sortmovies" v-bind:key="index" >
              <td>{{movie.title}}</td>
              <td>{{movie.rating}}</td>
            </tr>
          </tbody>
...
export default {
  ...
  computed: {
    sortmovies() {
     return this.ratingsInfo.filter(movie => movie.title.toLowerCase().match(this.search.toLowerCase()))
    }
  }
};
